...
`ssh` defines no resource of its own — `sshHost()` composes a
`Machine.ManagedBlock`. It is in the resource-package row because it owns a
domain, not because it adds a kind.
