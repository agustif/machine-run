...
**Nine naming conventions for 23 kinds** (`Machine.*`, `System.*`, `MacOS.*`,
...
(all seventeen have one). This section is the summary; those files carry the
...
✗ P2  ssh agent config — Ssh.Key and Ssh.KnownHost exist and machine already
      depends on ssh; agent forwarding is still only the raw ssh_config
      escape hatch, not a first-class resource.
...
oxlint            25 oxlint-plugin-effect rules, ALL enabled
                  0 errors · 736 warnings:
                    ...
                    noConditionalEmptyObjectSpread 56 · noNodeBuiltinImport 16
                    noRuntimeTypeof 14 · noUnknownParameters 4
                    unicorn(no-useless-spread) 1 · typescript(prefer-as-const) 1
...
`deploy-check` is the only job that runs `alchemy plan`, and passing it end to
end is the only real evidence any resource has been exercised by the engine.
...
| [V2-PLAN.md](./V2-PLAN.md) | current priority order |
