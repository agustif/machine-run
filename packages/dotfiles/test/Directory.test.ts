...
it.effect(
  "observe reports nothing for a directory that does not exist yet",
  () =>
...
it.effect(
  "observe fails with a typed error when a file occupies the path",
  () =>
    withTempDir((reconciler, target) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
...
it.effect(
  "matches is satisfied by any mode when the recipe does not constrain one",
  () =>
    Effect.gen(function* () {
      const reconciler = yield* makeDirectoryReconciler;
      const desired = yield* reconciler.desired({ path: "/tmp/whatever" });
      expect(
        reconciler.matches({ path: "/tmp/whatever", mode: 0o755 }, desired),
      ).toBe(true);
    }).pipe(Effect.provide(layer)),
);
...
it.effect("matches rejects a mode the recipe does constrain", () =>
  Effect.gen(function* () {
    ...
    expect(reconciler.matches({ path: "/tmp/whatever", mode: 0o755 }, desired)).toBe(
      false,
    );
    expect(reconciler.matches({ path: "/tmp/whatever", mode: 0o700 }, desired)).toBe(
      true,
    );
  }).pipe(Effect.provide(layer)),
);
