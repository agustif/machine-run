...
import { parseWingetExport, parseWingetList } from "../src/backends/windows/Winget.ts";
...
/**
 * The parsers, run against output captured moments earlier on the
 ...
 * notice. CI's Windows verification job sets these three variables to files it
...
const wingetExportFile = envPath("MACHINE_RUN_WINGET_EXPORT");
...
it.skipIf(Option.isNone(wingetListFile))("`winget list` still parses on this machine", () => {
  const ids = parseWingetList(
    Fs.readFileSync(
      Option.getOrElse(wingetListFile, () => ""),
      "utf8",
    ),
...
it.effect.skipIf(Option.isNone(wingetExportFile))(
  "`winget export` still parses on this machine",
  () =>
    Effect.gen(function* () {
      const entries = yield* parseWingetExport(
        Fs.readFileSync(
          Option.getOrElse(wingetExportFile, () => ""),
          "utf8",
        ),
      );
...
it.skipIf(Option.isNone(chocoListFile))("`choco list` still parses on this machine", () => {
  const names = firstTokens(
    lines(
      Fs.readFileSync(
        Option.getOrElse(chocoListFile, () => ""),
        "utf8",
      ),
